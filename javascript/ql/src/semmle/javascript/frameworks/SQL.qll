...
private module MySql {
  ...
  LocalApiGraph::Node createConnection() {
    result = mysql().getMember("createConnection").getReturn()
  }

  /** Gets a call to `mysql.createPool`. */
  LocalApiGraph::Node createPool() { result = mysql().getMember("createPool").getReturn() }
  ...
  private class QueryCall extends DatabaseAccess, DataFlow::MethodCallNode {
    QueryCall() {
      exists(LocalApiGraph::Node recv | recv = createPool() or recv = connection() |
        this = recv.getMember("query").getReturn().asDataFlowNode()
      )
    }
  ...
  class Credentials extends CredentialsExpr {
    ...
    Credentials() {
      exists(LocalApiGraph::Node call, string prop |
        ...
        call.asDataFlowNode().asExpr().(CallExpr).hasOptionArgument(0, prop, this) and
        (
...
private module Postgres {
  ...
  private class QueryCall extends DatabaseAccess, DataFlow::MethodCallNode {
    QueryCall() { this = [client(), newPool()].getMember("query").getReturn().asDataFlowNode() }
  ...
  class Credentials extends CredentialsExpr {
    ...
    Credentials() {
      exists(DataFlow::InvokeNode call, string prop |
        call = [client(), newPool()].asDataFlowNode() and
  ...
  private class QueryCall extends DatabaseAccess, DataFlow::MethodCallNode {
    QueryCall() {
      exists(string meth |
        ...
        meth = "run"
      |
        this = newDb().getMember(meth).getReturn().asDataFlowNode()
      )
    }
...
private module MsSql {
  ...
  private class QueryCall extends DatabaseAccess, DataFlow::MethodCallNode {
    QueryCall() { this = request().getMember(["query", "batch"]).getReturn().asDataFlowNode() }
  ...
  class Credentials extends CredentialsExpr {
    ...
    Credentials() {
      exists(DataFlow::InvokeNode call, string prop |
        (
          call = mssql().getMember("connect").getReturn().asDataFlowNode()
          or
          call = mssql().getMember("ConnectionPool").getInstance().asDataFlowNode()
        ) and
        this = call.getOptionArgument(0, prop).asExpr() and
        (
          prop = "user" and kind = "user name"
...
private module Sequelize {
  ...
  private class QueryCall extends DatabaseAccess, DataFlow::MethodCallNode {
    QueryCall() { this = newSequelize().getMember("query").getReturn().asDataFlowNode() }
  ...
  class Credentials extends CredentialsExpr {
    ...
    Credentials() {
      exists(NewExpr ne, string prop |
        ne = newSequelize().asDataFlowNode().asExpr() and
        (
...
private module Spanner {
  ...
  class TransactionRunCall extends SqlExecution {
    TransactionRunCall() {
      this = transaction().getMember(["run", "runStream", "runUpdate"]).getReturn().asDataFlowNode()
    }
  }
