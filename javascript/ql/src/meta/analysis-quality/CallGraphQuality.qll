...
predicate isExternalLibrary(string name) {
  ...
  name = "zlib" or
  // Modules with special recognition (see `externalNode`)
  name = "jquery" or
  name = "lodash"
}
 ...
 */
SourceNode externalNode(string moduleName) {
  result = moduleImport(moduleName) and
  not moduleName = "." + any(string s)
  or
  result = jquery() and
  moduleName = "jquery"
  or
  result = globalVarRef("_") and
  moduleName = "lodash"
  or
  result = externalNode(moduleName).getAPropertyRead()
  or
  result = externalNode(moduleName).getAnInvocation()
  or
  result = externalNode(moduleName).(InvokeNode).getCallback(_).getParameter(_)
}
...
class ExternalCall extends RelevantInvoke {
  ExternalCall() {
    ...
    (
      ...
      exists(string name |
        isExternalLibrary(name) and
        this = externalNode(name + any(string s))
      )
      or
