...
private class SinkSummarizingConfig extends DataFlow::Configuration {
  ...
  override predicate isSource(DataFlow::Node nd, DataFlow::FlowLabel lbl) {
    exists(LocalApiGraph::UseNode u |
      u = LocalApiGraph::root().getASuccessor().(LocalApiGraph::DefNode).getASuccessor*() and
      nd = u.asDataFlowNode()
    ) and
...
private class SourceSummarizingConfig extends DataFlow::Configuration {
  ...
  override predicate isSink(DataFlow::Node nd, DataFlow::FlowLabel lbl) {
    exists(LocalApiGraph::DefNode d |
      d = LocalApiGraph::root().getASuccessor().(LocalApiGraph::DefNode).getASuccessor*() and
      nd = d.asDataFlowNode()
    ) and
    lbl = any(DataFlow::FlowLabel l)
  }
...
query predicate sinkSummary(
  string tag, LocalApiGraph::UseNode u, DataFlow::FlowLabel lbl, DataFlow::Configuration base
) {
  tag = "sinkSummary" and
  exists(
  ...
  |
    ...
    u.asDataFlowNode() = source.getNode() and
...
query predicate sourceSummary(
  string tag, LocalApiGraph::DefNode d, DataFlow::FlowLabel lbl, DataFlow::Configuration base
) {
  ...
  |
    ...
    last = source.getASuccessor*() and
    sink = last.getASuccessor() and
    d.asDataFlowNode() = sink.getNode() and
    // avoid constructing infeasible paths
