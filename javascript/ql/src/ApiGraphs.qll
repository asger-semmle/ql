...
module ApiGraph {
  module Label {
    ...
    string parameter(int i) { result = "parameter " + i and i in [0 .. maxParm()] }
...
module LocalApiGraph {
  ...
  class Node extends Impl::TNode {
    ...
    Node getCorrespondingNode() {
      exists(string lbl | result = getAPredecessor(lbl).getCorrespondingNode().getASuccessor(lbl))
    }
    ...
    DataFlow::Node asDataFlowNode() { none() }
  ...
  class RootNode extends Node, Impl::MkRootNode {
    ...
    override RootNode getCorrespondingNode() { any() }
  }
  ...
  class DefNode extends Node {
    ...
    override DataFlow::Node asDataFlowNode() { this = Impl::MkDefNode(result) }

    override int getKind() { result = 1 }

    override UseNode getCorrespondingNode() { result = super.getCorrespondingNode() }
  }
  ...
  class UseNode extends Node {
    ...
    predicate flowsTo(DataFlow::Node nd) { Impl::trackUseNode(asDataFlowNode()).flowsTo(nd) }

    override DataFlow::SourceNode asDataFlowNode() { this = Impl::MkUseNode(result) }

    override int getKind() { result = 2 }

    override DefNode getCorrespondingNode() { result = Node.super.getCorrespondingNode() }
  }
...
module RemoteApiGraph {
  ...
  private predicate edge(string pred, string succ, string value) {
