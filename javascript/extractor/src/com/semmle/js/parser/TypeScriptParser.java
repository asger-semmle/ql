 ...
 *
 * The Node.js half of the wrapper is expected to live at
 * {@code $SEMMLE_DIST/tools/typescript-parser-wrapper/main.js}; non-standard
 * locations can be configured using the property {@link #PARSER_WRAPPER_PATH_ENV_VAR}.
 *
 * The script is started upon parsing the first TypeScript file and then is
 * kept running in the background, passing it requests for parsing files and
 * getting JSON-encoded ASTs as responses.
 */
public class TypeScriptParser {
	/**
	 * An environment variable that can be set to indicate the location of the
	 * TypeScript parser wrapper when running without SEMMLE_DIST.
	 */
	public static final String PARSER_WRAPPER_PATH_ENV_VAR = "SEMMLE_TYPESCRIPT_PARSER_WRAPPER";

	/**
	 * An environment variable that can be set to specify a timeout to use when
	 * verifying the TypeScript installation, in milliseconds. Default is 10000.
	 */
	public static final String TYPESCRIPT_TIMEOUT_VAR = "SEMMLE_TYPESCRIPT_TIMEOUT";

	/**
	 * An environment variable (without the <tt>SEMMLE_</tt> or <tt>LGTM_</tt>
	 * prefix), that can be set to indicate the maximum heap space usable by the
	 * Node.js process, in addition to its "reserve memory".
	 * <p>
	 * Defaults to 1.0 GB (for a total heap space of 1.4 GB by default).
	 */
	public static final String TYPESCRIPT_RAM_SUFFIX = "TYPESCRIPT_RAM";

	/**
	 * An environment variable (without the <tt>SEMMLE_</tt> or <tt>LGTM_</tt>
	 * prefix), that can be set to indicate the amount of heap space the Node.js
	 * process should reserve for extracting individual files.
	 * <p>
	 * When less than this amount of memory is available, the TypeScript compiler
	 * instance is restarted to free space.
	 * <p>
	 * Defaults to 400 MB (for a total heap space of 1.4 GB by default).
	 */
	...
	/**
	 * If non-zero, we use this instead of relying on the corresponding environment
	 * variable.
	 */
	...
	/**
	 * Verifies that Node.js and TypeScript are installed and throws an exception
	 * otherwise.
	 *
	 * @param verbose
	 *            if true, log the version strings and NODE_PATH.
	 */
	...
	public String verifyNodeInstallation() {
		...
		try {
			...
			if (r != 0 || stdout.length() == 0) {
				throw new CatastrophicError("Could not start Node.js. It is required for TypeScript extraction.\n" + stderr);
			}
			return nodeJsVersionString = stdout;
		} catch (InterruptedError e) {
			Exceptions.ignore(e, "Exception details are not important.");
			throw new CatastrophicError("Could not start Node.js (timed out after " + (timeout / 1000) + "s).");
			...
			throw new UserError("Could not start Node.js. It is required for TypeScript extraction."
	...
	private void setupParserWrapper() {
		...
		int mainMemoryMb = typescriptRam != 0 ? typescriptRam : getMegabyteCountFromPrefixedEnv(TYPESCRIPT_RAM_SUFFIX, 1000);
		...
		try {
			...
			throw new CatastrophicError("Could not start TypeScript parser wrapper "
					+ "(command: ." + parserWrapperCommand + ")", e);
		}
	}
	...
	private File getParserWrapper() {
		...
		if (semmleDistVar != null && !semmleDistVar.isEmpty()) {
			...
			throw new CatastrophicError("Could not find TypeScript parser: " + Env.Var.SEMMLE_DIST.name() + " is not set.");
		}
		if (!parserWrapper.isFile())
			throw new ResourceError("Could not find TypeScript parser: " +
					parserWrapper + " does not exist.");
		return parserWrapper;
	}

	/**
	 * Send a {@code request} to the Node.js parser wrapper process, and return
	 * the response it replies with.
	 */
	private JsonObject talkToParserWrapper(JsonObject request) {
		...
		if (!parserWrapperProcess.isAlive()) {
			...
			throw new CatastrophicError("TypeScript parser wrapper terminated with exit code " +
					exitCode + "; stderr: " + err);
		}
		...
		try {
			...
			throw new CatastrophicError(
					...
					response + " (command: " + parserWrapperCommand + ").", e);
		}
	}
	 ...
	 * project, although this is not yet implemented.
	 * <p>
	 * If the file is not part of a project, only syntactic information will be
	 * extracted.
	 */
	 ...
	 * <p>
	 * Only one project should be opened at once.
	 */
	public ParsedProject openProject(File tsConfigFile) {
		...
		try {
			...
			throw new CatastrophicError("TypeScript parser wrapper sent unexpected response: " + response, e);
		}
	}
	...
	/**
	 * Closes any open project, and in general, brings the TypeScript wrapper to a
	 * fresh state as if it had just been restarted.
	 * <p>
	 * This is to ensure tests are isolated but without the cost of restarting the
	 * Node.js process.
	 */
	public void reset() {
		try {
	...
	private void checkResponseType(JsonObject response, String type) {
		...
		if (typeElm == null || !typeElm.getAsString().equals(type)) {
			throw new CatastrophicError("TypeScript parser sent unexpected response: " + response + ". Expected " + type);
		}
	}
	 ...
	 *
	 * A new process will be started the next time a request is made.
	 */
