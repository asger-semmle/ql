...
public class Main {
	/**
	 * A version identifier that should be updated every time the extractor changes in
	 * such a way that it may produce different tuples for the same file under the same
	 * {@link ExtractorConfig}.
	 */
	public static final String EXTRACTOR_VERSION = "2019-01-23";
	 ...
	 *
	 * This is used by the test runner.
	 */
	...
	public void run(String[] args) {
		...
		if (ap.has(P_TRAP_CACHE)) {
			...
			if (ap.has(P_TRAP_CACHE_BOUND)) {
				...
				if (sizeBound == null)
					ap.error("Invalid TRAP cache size bound: " + tcb);
			}
	...
	private ArgsParser addArgs(ArgsParser argsParser) {
		argsParser.addDeprecatedFlag(P_ECMA_VERSION, 1, "Files are now always extracted as ECMAScript 2017.");
		argsParser.addFlag(P_EXCLUDE, 1, "Do not extract files matching the given filename pattern.", true);
		...
		argsParser.addFlag(P_FILE_TYPE, 1, "Assume all files to be of the given type, regardless of extension; "
				+ "the type must be one of " + StringUtil.glue(", ", FileExtractor.FileType.allNames) + ".");
		argsParser.addFlag(P_HELP, 0, "Display this help.");
		argsParser.addFlag(P_HTML, 1, "Control extraction of HTML files: "
		...
		argsParser.addFlag(P_INCLUDE, 1, "Extract files matching the given filename pattern (in addition to HTML and JavaScript files).", true);
		...
		argsParser.addFlag(P_PLATFORM, 1, "Extract the given JavaScript files as code for the given platform: "
		...
		argsParser.addFlag(P_SOURCE_TYPE, 1, "The source type to use; must be one of 'script', 'module' or 'auto'. "
				+ "The default is 'auto'.");
		argsParser.addToleratedFlag(P_TOLERATE_PARSE_ERRORS, 0);
		argsParser.addFlag(P_ABORT_ON_PARSE_ERRORS, 0, "Abort extraction if a parse error is encountered.");
		...
		argsParser.addFlag(P_TYPESCRIPT_FULL, 0, "Enable full TypeScript support with static type information.");
		argsParser.addFlag(P_TYPESCRIPT_RAM, 1, "Amount of memory allocated to the TypeScript compiler process. The default is 1G.");
	...
	private static TypeScriptMode getTypeScriptMode(ArgsParser ap) {
		if (ap.has(P_TYPESCRIPT_FULL))
			return TypeScriptMode.FULL;
		if (ap.has(P_TYPESCRIPT))
			return TypeScriptMode.BASIC;
		return TypeScriptMode.NONE;
	}

	private ExtractorConfig parseJSOptions(ArgsParser ap) {
		ExtractorConfig cfg = new ExtractorConfig(enableExperimental(ap))
				.withExterns(ap.has(P_EXTERNS))
				.withPlatform(getPlatform(ap))
				.withTolerateParseErrors(ap.has(P_TOLERATE_PARSE_ERRORS) || !ap.has(P_ABORT_ON_PARSE_ERRORS))
				.withHtmlHandling(ap.getEnum(P_HTML, HTMLHandling.class, ap.has(P_EXPERIMENTAL)? HTMLHandling.ALL: HTMLHandling.ELEMENTS))
				...
				.withTypeScriptMode(getTypeScriptMode(ap)).withTypeScriptRam(
						ap.has(P_TYPESCRIPT_RAM) ? UnitParser.parseOpt(ap.getString(P_TYPESCRIPT_RAM), UnitParser.MEGABYTES) : 0);
		if (ap.has(P_DEFAULT_ENCODING))
			cfg = cfg.withDefaultEncoding(ap.getString(P_DEFAULT_ENCODING));
		return cfg;
	}
	...
	/**
	 * Collect files to extract under a given root, which may be either a file
	 * or a directory. The {@code explicit} flag indicates whether {@code root}
	 * was explicitly passed to the extractor as a command line argument, or
	 * whether it is examined as part of a recursive traversal.
	 */
		...
		if (root.isDirectory()) {
			...
			if (seenDirectories.add(FileUtil.tryMakeCanonical(root).getPath()))
				...
				if (!excludeMatcher.matches(root.getAbsolutePath())) {
					...
					if (gs == null)
						System.err.println("Skipping " + root + ", which cannot be listed.");
					else
						for (File g : gs)
							collectFiles(g, false);
				}
		} else {
			...
			if (fileExtractor.supports(root) && extractorOutputConfig.shouldExtract(root) &&
					(explicit || includeMatcher.matches(path) && !excludeMatcher.matches(path))) {
				files.add(normalizeFile(root));
			}

			if (extractorConfig.getTypeScriptMode() == TypeScriptMode.FULL && root.getName().equals("tsconfig.json")
